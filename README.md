...
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=FFD700,FFA500&height=300&section=header&text=GOLDNet&fontSize=90" />
</p>
...
<details>
<summary><strong>1. AI 기반 데이터 파싱 시스템 구축</strong></summary>

- **문제**: 웹 크롤링으로 수집한 채용 공고는 형식이 제각각인 **비정형 텍스트** 데이터라 정형화된 DB 스키마에 저장하기 어려웠습니다.
- **해결**: **OpenAI GPT-4o** 모델을 활용하여 공고 내용을 분석하고, 직무, 자격요건, 급여 등을 정확히 추출하여 JSON 형식으로 구조화하는 지능형 파서를 구현했습니다.
- **성과**: 데이터 정제 및 입력 과정을 **100% 자동화**하여 휴먼 에러를 최소화하고 데이터 품질을 극대화했습니다.

</details>

<details>
<summary><strong>2. Flyway를 이용한 DB 스키마 버전 관리</strong></summary>

- **문제**: 애자일 개발 환경에서 DB 스키마 변경이 잦아 팀원 간 개발 환경이 불일치하고, 배포 시 DB 동기화 문제가 발생했습니다.
- **해결**: **Flyway**를 도입하여 모든 DB 스키마 변경 이력을 SQL 스크립트로 관리하고 버전을 부여했습니다. 애플리케이션 실행 시 자동으로 최신 버전으로 마이그레이션됩니다.
- **성과**: 팀 협업의 안정성을 확보하고, 배포 과정에서 발생할 수 있는 DB 관련 이슈를 원천적으로 제거했습니다.

</details>

<details>
<summary><strong>3. GitHub Actions 기반 CI/CD 파이프라인 구축</strong></summary>

- **문제**: 수동으로 코드를 빌드, 테스트, 배포하는 과정은 시간이 많이 소요되고 실수가 발생할 가능성이 높았습니다.
- **해결**: **GitHub Actions** 워크플로우를 작성하여, Main 브랜치에 코드가 Push될 때마다 자동으로 빌드, 테스트, Docker 이미지 생성 및 EC2 서버 배포가 이루어지도록 파이프라인을 구축했습니다.
- **성과**: 배포 과정을 완전히 자동화하여 개발 생산성을 극대화하고, 신속하고 안정적인 서비스 운영이 가능해졌습니다.
...
### B. 기술적 성과
- **비정형 데이터 처리 혁신**: GPT-4o를 활용한 자연어 처리 기술로 웹 데이터 정형화에 성공, 일관된 데이터 품질을 확보했습니다.
- **확장 가능한 아키텍처**: MSA(Microservices Architecture)를 고려한 모듈화된 설계로, 향후 새로운 채용 사이트나 AI 모델을 유연하게 추가할 수 있습니다.
- **DevOps 문화 내재화**: Docker를 통한 환경 일관성 확보 및 GitHub Actions 기반의 완전 자동화된 배포 파이프라인을 구축하여 안정적인 개발 및 운영 체계를 마련했습니다.

### C. 사회적 가치
- **디지털 포용성 증진**: 고령층의 정보 격차를 해소하고 디지털 세상에 더 쉽게 참여할 수 있는 발판을 마련했습니다.
- **일자리 매칭 효율성 증대**: AI 추천을 통해 구직에 소요되는 시간과 노력을 단축시켜 고령층의 경제 활동을 지원합니다.
- **사회 참여 기회 확대**: 안정적인 일자리를 통해 고령층이 사회의 일원으로서 활발하게 기여할 수 있는 기회를 넓혔습니다.
...
| 역할 | 이름 | 담당 업무 | GitHub |
| :--- | :--- | :--- | :--- |
| 👑 **팀장** | **김지현** | 프로젝트 총괄, 기획, 백엔드 개발 | `[@github-id]` |
| 💻 **팀원** | **강지수** | 기획, 프론트엔드 개발 | `[@github-id]` |
| 📊 **팀원** | **김이림** | 기획, 데이터 수집 및 분석 | `[@github-id]` |
| 📈 **팀원** | **박지우** | 기획, 데이터 수집 및 분석 | `[@github-id]` |
| 🔎 **팀원** | **정선영** | 기획, 데이터 수집 및 분석 | `[@github-id]` |
| 멘토 | 김기태 | 프로젝트 멘토링 | - |
---
